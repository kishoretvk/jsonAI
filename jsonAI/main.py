...
class Jsonformer:
    ...
    def __init__(
        ...
        *,
        ...
        self.integer_logit_processor = OutputIntegersTokens(
            self.tokenizer, self.prompt
        )
    ...
    def generate_enum(self, enum_values: Set[str]) -> str:
        ...
        for option in enum_values:
            ...
            termination_probability = torch.max(
                probabilities[-1, terminal_tokens]
            )
            option_probability = (
                torch.prod(option_token_probabilities) * termination_probability
            )
            self.debug("[generate_enum]", f"{option_probability}, {option}")  # E261 fix: added space
